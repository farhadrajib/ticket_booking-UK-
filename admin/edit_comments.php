...
require_once("../includes/database.php");
	...
	if(isset($update))
	{
	   mysql_query("update user_comments set title='$title',details='$details',status='$status' WHERE id='$id'");
	} 
	
	  $sql_comments=mysql_query("select *from user_comments WHERE id='$id'");
	
	  $array=@mysql_fetch_array($sql_comments) ;
	 
	 	   $array_title=$array['title'];
...
<form id="form1" name="form1" method="post" action="">
  <table width="264" border="0" align="center">
    ...
    <tr>
      ...
      <td colspan="2"><input type="text" name="title" id="title" value="<?php echo $array_title;?>" /> </td>
    </tr>
    <tr>
      <td class="header-text">Detailas</td>
      <td colspan="2"><textarea name="details" id="details"><?php echo $details;?></textarea></td>
    </tr>
    <tr>
      <td class="header-text">Status</td>
      <td width="89"><input name="status" type="radio" id="radio" value="1" checked="checked" />
        Published</td>
      <td width="104"><input type="radio" name="status" id="radio2" value="0" />
        Unpublished</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td colspan="2"><p>
        ...
        <p align="right"><a href="user_comments.php" class="header-text">back</a></p></td>
    </tr>
  </table>
